...
def update_docket(docket, user):
	frappe.db.sql("""Update `tabSales Order` set flow_status = 'Digital Room', received_by = %s
			        where name = %s""", (user, docket))
	frappe.db.commit()
	print "Im Here++++++++"
	# frappe.msgprint("Successfully Received!")
	return frappe.msgprint("Successfully Received Docket " + docket + " in Digital Room!")
