...
def execute(filters=None):
	...
	wd = frappe.db.sql("""Select transaction_date, name, customer_name, quote_no, lpo_no, flow_status, received_by
							  from `tabSales Order` where docstatus = 0 and name = %s and flow_status != 'Job Done' """, (work_docket))
...
def update_docket(docket, user):
	frappe.db.sql("""Update `tabSales Order` set flow_status = '75/74/72', received_by = %s
			        where name = %s""", (user, docket))
	frappe.db.commit()
	print "Im Here++++++++"
	# frappe.msgprint("Successfully Received!")
	return frappe.msgprint("Successfully Received Docket " + docket + " in 75/74/72 Machine!")
