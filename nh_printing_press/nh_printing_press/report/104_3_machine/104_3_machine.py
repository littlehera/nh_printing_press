...
def update_docket(docket, user):
	frappe.db.sql("""Update `tabSales Order` set flow_status = '104/3 Machine', received_by = %s
			        where name = %s""", (user, docket))
	frappe.db.commit()
	print "Im Here++++++++"
	# frappe.msgprint("Successfully Received!")
	return frappe.msgprint("Successfully Received Docket " + docket + " in 104/3 Machine!")
